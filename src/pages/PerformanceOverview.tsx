...
const PerformanceOverview = () => {
  ...
  const distBuckets = [
    { name: "Kiváló (90-100%)", min: 90, max: 101, color: COLORS[2] },
    { name: "Jó (75-89%)", min: 75, max: 90, color: COLORS[6] },
    { name: "Megfelelő (60-74%)", min: 60, max: 75, color: COLORS[3] },
    { name: "Elégtelen (<60%)", min: 0, max: 60, color: COLORS[0] },
  ];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="space-y-3">
            ...
            <Tabs value={activeView} onValueChange={(v) => setActiveView(v as "overview" | "users")}>
              <TabsList className="h-9">
                <TabsTrigger value="overview" className="gap-1.5 text-sm"><BarChart3 className="w-4 h-4" /> Áttekintés</TabsTrigger>
                <TabsTrigger value="users" className="gap-1.5 text-sm"><Users className="w-4 h-4" /> Felhasználószintű</TabsTrigger>
              </TabsList>
            </Tabs>
        ...
        <div className="grid gap-4 grid-cols-2 lg:grid-cols-5">
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{totalStudents}</p><p className="text-xs text-muted-foreground">Aktív hallgató</p></div>
          ...
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{totalExams}</p><p className="text-xs text-muted-foreground">Összes vizsga</p></div>
          </div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{avgScore.toFixed(1)}%</p><p className="text-xs text-muted-foreground">Átlag pontszám</p></div>
          </div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{passRate.toFixed(0)}%</p><p className="text-xs text-muted-foreground">Sikerességi arány</p></div>
          </div></CardContent></Card>
          <Card><CardContent className="pt-6"><div className="flex items-center gap-3">
            ...
            <div><p className="text-2xl font-bold">{badgeCount}</p><p className="text-xs text-muted-foreground">Kiosztott jelvény</p></div>
          </div></CardContent></Card>
        </div>
                ...
                <>
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
                      ...
                      <Tooltip contentStyle={tooltipStyle} formatter={(value: number, name: string) => [`${value} vizsga`, name]} />
                    </PieChart>
                  </ResponsiveContainer>
              ...
              {courseStats.length > 0 ? (
                <ResponsiveContainer width="100%" height={Math.max(200, courseStats.length * 45)}>
                  <BarChart data={courseStats} layout="vertical">
                    ...
                    <Tooltip contentStyle={tooltipStyle} formatter={(value: number, _: string, props: any) => [
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {topPerformers.map((p, i) => (
                      <tr key={p.user_id} className="border-b last:border-0 hover:bg-muted/30 group cursor-pointer" onClick={() => navigate(`/performance/${p.user_id}`)}>
                        <td className="py-2.5">
