-- Phase 2: Performance Calculation Database Functions

-- Function 1: Calculate Exam Performance
-- Returns average score of successful exams (score >= 80) within date range
...
-- Function 2: Calculate Training Activity Percentage
-- Returns percentage of completed trainings vs available trainings
-- Special case: START program completion = 100% for first year employees
...
BEGIN
  ...
  -- Special case: First year employees with START program completion = 100%
  IF v_is_first_year AND v_has_start_program THEN
    RETURN 100.00;
  END IF;
  
  -- Count completed required trainings in period
  SELECT COUNT(*)
  ...
  -- Count available required trainings in period
  SELECT COUNT(*)
...
-- Returns decimal years of service based on start_of_empl
...
-- Function 4: Update User Performance Metrics
-- Orchestrates all calculations and updates/inserts metrics table
...
BEGIN
  -- Determine date range based on evaluation period
  ...
  INSERT INTO public.user_performance_metrics (
    ...
    period_start,
    period_end,
    exam_performance_pct,
    training_activity_pct,
    overall_performance_pct,
    successful_exams_count,
    total_exams_count,
    completed_trainings_count,
    available_trainings_count,
    years_of_service,
    updated_at
  ) VALUES (
    p_user_id,
    ...
    v_successful_exams,
    v_total_exams,
    v_completed_trainings,
    v_available_trainings,
    v_years_service,
    NOW()
  )
...
-- Function 5: Batch Update All Users Performance Metrics
-- Helper function to update metrics for all active users
...
GRANT EXECUTE ON FUNCTION public.calculate_exam_performance TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.calculate_training_activity TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.calculate_years_of_service TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.update_user_performance_metrics TO authenticated, anon;
GRANT EXECUTE ON FUNCTION public.update_all_users_performance_metrics TO authenticated, anon;